...
async def login(request: Request,  username: str = Form(...), password: str = Form(...)):
    ...
    if user: 
        if verify_user(user[1],password):
            request.session["username"] = user[1]
            request.session["admin"]=user[3]
            return templates.TemplateResponse("home.html", {"request":request, "id": user[1], "admin": user[3]})
        else:
            return templates.TemplateResponse("failure.html", {"request": request, "error": "Invalid password"})
    else:
        return templates.TemplateResponse("failure.html", {"request": request, "error": "Invalid Username"})
